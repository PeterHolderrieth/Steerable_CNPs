...
def unstable_eig_val_cov_converter(X,activ_type="softplus"):
    '''
    Input:
...
def eig_val_cov_coverter(X,activ_type="softplus",tol=1e-7):
    ...
    if torch.sum(above_tol)>0:
        Out[above_tol]=unstable_eig_val_cov_converter(X[above_tol],activ_type=activ_type)
    if activ_type=="softplus":
...
def cov_converter(Pre_Sigma_Grid,dim_cov_est):
    '''
